...
object Boxes {
    ...
    fun getBox(width: Int, height: Int, cornerColor: Color, barColor: Color): BufferedImage {
        var svg = javaClass.getResourceAsStream("/box.svg").bufferedReader().readLines().joinToString("\n")
        svg = svg.replace("%PRIMARY_COLOR%", "#${Integer.toHexString(cornerColor.rgb).substring(2)}")
        svg = svg.replace("%SECONDARY_COLOR%", "#${Integer.toHexString(barColor.rgb).substring(2)}")
